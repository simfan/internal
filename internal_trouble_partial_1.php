		...
		if($password == $user_info['car_password'])
		{
			for($i = 0; $i < 4; $i++)
			{
				...
				if ($user_info[$plant_variable] != ' ' && $user_info[$plant_variable] != '' && !is_null($user_info[$plant_variable]) )
				{
					...
					if ($plant_letter != 'A')
					{
						$plant_location[$i] = $user_info[$plant_variable];
    ...
    {?>
		...
		<?php 
			if ($_GET['action'] == 'add')
			{
				...
				print "<td class = '" . $picture_class . "' ><input type = 'file' name = 'picture1' id = 'picture1' ></br><input type = 'file' name = 'picture2' id = 'picture2' ></br><input type = 'file' name = 'picture3' id = 'picture3' ></td>";
			}?>
			</tr>	
